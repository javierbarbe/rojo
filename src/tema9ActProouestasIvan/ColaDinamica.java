...
class ColaDinamica<E> {
	...
	public void aniadePrincipio(Object o) {
		try {
			...
			// si no hubiera sido inicializado nuestro array, por defecto se asume que
			// tamanio=1
			if (tamanio > 0 && tamanio != null) {
				tamanio += 1;
			}
		...
		aux = new Object[tamanio];
		// aqui inicializamos el array Default en caso de que tamaño sea 1, (el menor
		// tamaño posible para un array)
		if (tamanio < 2) {
			arrDefault = new Object[tamanio];
		}
	...
	public void eliminarNodo(int posicion) {
		...
		for (int h = 0; h < arrDefault.length; h++) {
			...
			// antes de recorrer el array, puesto que queremos eliminarlo, lo que vamos a
			// hacer
			// es no añadirlo
